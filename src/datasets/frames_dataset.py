...
def read_video(name, frame_shape):
    ...
    if os.path.isdir(name):
        ...
        name = '/data/kevinhuang/github/articulated-animation/data/FVUSM_256x256_gif/train/0582.gif'
        video = mimread(name)
        # if len(video[0].shape) == 2:
        #     video = [gray2rgb(frame) for frame in video]
        for i in range(len(video)):
            if video[i].shape[-1] == 4:
                video[i] = video[i][..., 0]
        if frame_shape is not None:
...
class FramesDataset(Dataset):
    ...
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True,
        ...
        if self.is_train:
            self.transform = None # AllAugmentationTransform(**augmentation_params)
        else:
            self.transform = None
