...
class FunctionNegativeTripletSelector(TripletSelector):
    ...
    negative_selection_fn should take array of loss_values for a given anchor-positive pair and all
    negative samples and return a negative index for that pair
    """
    ...
    def get_triplets(self, embeddings, labels):
        ...
        for label in set(labels):
            ...
            anchor_positives = np.array(
                ...
                list(combinations(label_indices[::-1], 2)))  # All anchor-positive pairs
            ap_distances = distance_matrix[anchor_positives[:, 0], anchor_positives[:, 1]]
            for anchor_positive, ap_distance in zip(anchor_positives, ap_distances):
                if self.is_distance:
                    loss_values = ap_distance - distance_matrix[anchor_positive[0],
                    ...
                    # loss_values = ap_distance - np.array([an_distance, pn_distance]).min(0) + \
                    # self.margin
                else:
                    loss_values = distance_matrix[anchor_positive[0],
                                                  negative_indices] - ap_distance + self.margin
...
def HardestNegativeTripletSelector(margin, is_distance=True, cpu=True):
    return FunctionNegativeTripletSelector(
        margin=margin, negative_selection_fn=hardest_negative, is_distance=is_distance, cpu=cpu)


def RandomNegativeTripletSelector(margin, is_distance=True, cpu=True):
    return FunctionNegativeTripletSelector(
        margin=margin, negative_selection_fn=random_hard_negative, is_distance=is_distance, cpu=cpu)


def SemihardNegativeTripletSelector(margin, is_distance=True, cpu=True):
    return FunctionNegativeTripletSelector(
        margin=margin,
